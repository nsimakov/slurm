...
_rpc_abort_job(slurm_msg_t *msg)
{
	...
	task_g_slurmd_release_resources(req->step_id.job_id);
...
/* 	if (shm_update_job_timelimit(req->step_id.job_id, req->expiration_time) < 0) { */
/* 		error("updating lifetime for job %u: %m", req->step_id.job_id); */
/* 		rc = ESLURM_INVALID_JOB_ID; */
/* 	} else */
/* 		debug("reset job %u lifetime", req->step_id.job_id); */
...
static int _compare_starting_steps(void *listentry, void *key)
{
	...
	return memcmp(step0, step1, sizeof(*step0));
}
